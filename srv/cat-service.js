...
module.exports = async srv => {
	const sf = await cds.connect.to('externalService')
	...
	myMessaging.on('topic:sfemessage', msg => {
		const { message, employeeId, managerId, readStatus } = msg.headers
		return cds.run(
	...
	srv.after('READ', 'Mappings', (mappings, req) => {
		...
		return Promise.all(
			mappings
				...
				.map(mapping => Promise.all([getEmployeeName(mapping, tx)]))
		)
	})
